...
- **Шаги:**
  1. Стейкхолдер нажимает `Создать кейс` и заполняет форму результата: название, краткое описание, блоки «проблема/цель/что сделали/итог», показатели результата, материалы, внешняя ссылка.
  2. Кейс сохраняется как `Черновик` и доступен только внутри кабинетов/ролей (не в публичной витрине).
  3. Стейкхолдер публикует кейс (`Опубликовать`) → кейс становится видимым гостям и попадает в публичную ленту `Портфель проектов`.
  4. При необходимости стейкхолдер снимает кейс с публикации (`Снять с публикации`, обязательна причина) — кейс исчезает из публичной витрины.
- **Артефакты:** карточка кейса (статусы публикации `Черновик/Опубликовано/Снято с публикации`), публичная страница `case-public.html`, материалы (список документов), блок `Команда` (из Bitrix24), `Внешняя ссылка`.

## 9. Публичный портфель проектов (Фаза 3)
- **Участники:** гость/авторизованный пользователь (просмотр).
- **Шаги:**
  1. Пользователь открывает страницу `Портфель проектов` (лента опубликованных кейсов).
...
- **Шаги:**
  ...
  3. После регистрации пользователь видит запись в ЛК `События` и может `Отменить` регистрацию; доступна кнопка `Добавить в календарь (.ics)`.
...
5. **Архив → (опционально) Кейс → Публичная витрина** (создание кейса, публикация, показ в «Портфеле проектов»).
