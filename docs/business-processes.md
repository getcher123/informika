...
- **Шаги:**
  1. После статуса «Одобрена» участники на странице идеи видят кнопку «Вступить в команду» и заполняют модальное окно (роль, уровень компетенции, опыт, портфолио, согласие).
  2. Заявка появляется в ЛК участника (вкладка «Заявки в команду») и у координатора: карточки заявок имеют кнопки «Принять», «Отклонить», «Подробнее».
  3. Координатор открывает карточку заявки (панель с полями роли, компетенции, опыта) и принимает решение (включая опцию «Запросить уточнение»). Принятая заявка добавляет участника в блок «Моя команда»; отклонённая уходит в архив с причиной.
  4. Координатор может приглашать участников из CRM (вкладка «Кандидаты») и закреплять обязательные роли (вкладка «Команда»).
  5. Стейкхолдер видит состав и комплектность ролей в портфеле; при необходимости инициирует административные задачи.
- **Артефакты:** модальное окно «Вступить в команду», вкладка «Заявки в команду», карточка заявки (подраздел 6.3.1), CRM координатора (вкладки «Команда», «Кандидаты», «Журнал»), блок «Команда» на странице идеи.
...
- **Шаги:**
  ...
  2. Управление документами через кнопку «Перейти к документам»/Диск идеи (структура `/Требования`, `/План`, `/Прототипы`, `/Тесты`, `/ПК-НТК`, `/Отчёты`, `/Коммуникации`).
...
- **Шаги:**
  1. Координатор использует чек-лист готовности (описан в `role-checklists.md`) и отмечает выполнение пунктов в административной карточке.
  2. Документы загружаются в папку `/ПК-НТК`; статус стадии на карточке в портфеле переключается на «Готово к ПК» или «Готово к НТК» (визуальный бейдж на карточке идеи стейкхолдера).
  3. Стейкхолдер проверяет материалы через кнопки «Перейти к документам» и утверждает готовность.
  4. Проведение комиссий (ПК/НТК) фиксируется в комментариях и административных задачах; при необходимости добавляются заметки в вкладке «Уведомления».
- **Артефакты:** чек-листы готовности, папка `/ПК-НТК`, бейджи стадий на карточках, задачи административной доски.

## 7. Перевод в «Архив» и отчётность
- **Участники:** стейкхолдер, координатор, система уведомлений.
- **Шаги:**
  1. После подтверждения результатов (ПК/НТК) идея переводится в статус «Архив» (см. [statuses-table.md](statuses-table.md)).
