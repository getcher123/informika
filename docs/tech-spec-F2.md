...
- Стейкхолдер: в «Портфеле идей» видит все идеи по своим запросам (включая одобренные/архивные), фильтрует по статусу, открывает карточку идеи и переходит в Bitrix24; заполненность команды отображается как `X/Y` обязательных ролей.
