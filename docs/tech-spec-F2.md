...
| ------------------------- | -------------------------------------------------------------------------------------------------------- |
| Стейкхолдер               | Просмотр портфеля одобренных идей, просмотр отчётов, принятие решений по идеям, комментарии.             |
...
- в портфеле одобренных идей визуализировать только статус «Одобрена» и обеспечивать быстрые переходы к задачам/документам/канбану идеи. Подготовка материалов и подтверждение готовности к ПК/НТК выполняются исключительно в задачах Bitrix24 и не отображаются отдельными бейджами или фильтрами на витрине.
...
2. При отправке на доработку причина обязательна; событие фиксируется в журнале идеи, уведомления получают автор и координатор. Повторная отправка возможна только после внесения правок.
3. Формирование команды выполняется только в Bitrix24: координатор принимает/отклоняет заявки, закрывает обязательные слоты через задачи/чек-листы. На портале отображается только текущий состав команды и статусы заявок (без индикаторов заполненности).
4. Любой переход статуса идеи (включая «Архив») фиксируется в журнале и отправляет уведомления всем заинтересованным ролям. На карточках выводится один бейдж дедлайна с датой `ДД.ММ.ГГГГ` или текстом «Просрочено».
5. Уведомления в кабинетах имеют одну кнопку `Перейти`, ведущую на карточку идеи/запроса или задачу Bitrix24; фильтры работают по четырём типам: идеи, команда, Bitrix, системные.
...
- Стейкхолдер: в «Портфеле идей» видит все идеи по своим запросам (включая одобренные/архивные), фильтрует по статусу, открывает карточку идеи и переходит в Bitrix24;
...
- Координатор: получает задачу в Bitrix24 «Настроить рабочую область» после одобрения идеи, закрывает обязательные роли через чек-листы, принимает/отклоняет заявки; портал отражает статусы заявок и заполненность слотов.
