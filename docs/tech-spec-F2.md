...
- Провести унификацию статусов, ролей и артефактов с Фазой 1 и расширенным ТЗ (`informika-extended-tz.md`).
...
5. Расширенные уведомления и автоматизации для этапов Фазы 2 (см. `automation-table.md`, `notification-table.md`).
6. Актуализация матрицы доступа и статусов (см. `access-matrix.md`, `statuses-table.md`).
...
- Используются статусы из `statuses-table.md` (идея, запрос, заявка, задача Bitrix24).
- Переход «Одобрена → Архив» инициируется после завершения работ и подготовки пакета документов для ПК/НТК.
- Статус «Архив» фиксирует превращение идеи в проект вне платформы (см. `README.md`, §3).
...
- Карточка одобренной идеи: вкладки «Обзор», «Задачи», «Документы», «Команда», «Решения», «История».
  ...
  - «Команда» выводит состав участников и их роли (источник данных — `informika-extended-tz.md`, §4.9).
...
- При статусе идеи «Одобрена» автоматически создаётся рабочая область: папки на Диске, шаблоны задач, чат (см. `automation-table.md`).
...
3. Отмечает готовность к ПК → система отправляет уведомления команде (см. `notification-table.md`).
...
## 7. UI/UX требования
- Навигация и элементы базируются на описании в `interface-elements.md` (разделы «ЛК стейкхолдера», «Bitrix24», «Уведомления»).
...
- Дополняются сущности «Проект» и «Профиль» согласно §10 `informika-extended-tz.md`.
...
1. Перевод идеи в «Одобрена» автоматически создаёт область в Bitrix24 и отправляет уведомления (см. `automation-table.md`).
...
- Расширенное ТЗ: `informika-extended-tz.md`
- ТЗ Фазы 1: `tech-spec-F1.md`
- Матрица доступа: `access-matrix.md`
- Таблица автоматизаций: `automation-table.md`
- Таблица уведомлений: `notification-table.md`
- Таблица статусов сущностей: `statuses-table.md`
- Описание интерфейсов: `interface-elements.md`
- Отчёт Фазы 1: `reports/phase1-summary.md`
