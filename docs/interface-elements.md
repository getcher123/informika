...
> - Цветные бейджи применяются только для направления (`Строительство`, `Эксплуатация`, `Безопасность`, `Цифровизация`, `Клиентский сервис`, `Другое`), времени (`Сегодня`, `Сбор идей до ...`, `Дедлайн ...`) и дедлайнов (оттенки: `Норма`, `Предупреждение`, `Просрочено`).
...
> - Индикатор `+N` всегда показывает количество новых событий внутри карточки и располагается.
...
- Кнопки: `Сохранить изменения` (основная), `Удалить`, `Отмена`.
...
- Кнопки: `Сохранить изменения` (основная), `Удалить`, `Отмена`. После сохранения — возврат в карточку идеи.
...
- Табличный вид идей автора:
  - Колонки: `Идея` (название), `Статус` (круглый индикатор), `Направление` (бейдж), `Дата публикации`, `Дедлайн` (бейдж срока), `Обновления` (`+N`).
...
- Боковое меню/вкладки: `Мои запросы`, `Портфель идей`, `Уведомления`, `Админ. панель`, `Профиль`. На пункте «Мои запросы» отображается индикатор `+N`, если есть новые идеи.

### 7.2 Вкладка «Мои запросы»

- Табличный реестр запросов:
  - Колонки: `Запрос` (название + индикатор `+N`), `Статус` (круглый индикатор `На сборе идей`/`Закрыт`), `Направление` (цветной бейдж), `Дедлайн` (бейдж `Сегодня`, `7 дней`, `Просрочено`), `Идей всего`, `Новых`, `Последнее обновление`.
...
- **Карточка неодобренной идеи** (статусы `На модерации`, `На доработке`, `На рассмотрении`, `Отклонена`):
  ...
  - Средний блок: краткое описание (до 2 строк), дата подачи / обновления, бейдж дедлайна.
  - Нижний блок: кнопки `Подробнее` (переход к описанию идеи внутри каталога), `Открыть запрос`, индикатор `Новая`. Карточки выполнены в нейтральной палитре.
- **Карточка одобренной идеи** (статусы `Одобрена`, `Архив`):
  - Верх: название, круглый индикатор `Одобрена/Архив`, бейдж направления, индикатор `+N`новых событий.
  - Средний блок: описание (до 2 строк), текстовые метрики `Команда: заполнено X обязательных ролей`, `Просрочено задач: …`, бейдж дедлайна.
...
- **Раскрытая страница идеи из портфеля** (переход по кнопке `Просмотр/Открыть`):
  ...
  - Блок «Описание»: краткое описание, ссылка на запрос-источник (`Открыть запрос`), дата публикации и дата обновления, индикатор `+N` новых событий.
...
- Всплывающие подсказки (tooltip) на статусах и бейджах: `+N` → «Новые идеи/обновления, которые вы ещё не просмотрели».
