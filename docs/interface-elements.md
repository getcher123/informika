...
- Поля повторяют структуру HTML-макета:
  ...
  4. `Основные этапы реализации идеи*` — textarea, подсказка «Напишите основные этапы, которые необходимы для реализации вашего запроса».
  5. `Ожидаемый результат*` — textarea, подсказка «Напишите ожидаемый результат от реализации вашего запроса».
  6. `Есть ли у вас наработки по данному запросу?*` — радиокнопки `Да`/`Нет`.
  7. `Файлы к запросу` — drag&drop-зона (`file-upload`): кнопка выбора + подпись «Выберите файлы или перетащите их сюда», поддерживается множественная загрузка.
