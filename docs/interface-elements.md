...
- Табличный вид идей автора:
  - Колонки: `Идея` (название), `Статус` (круглый индикатор), `Направление` (бейдж), `Дата публикации`, `Дедлайн` (бейдж срока), `Новые` (бейдж `+N`).
  - Действия в строке: `Просмотр` (открывает карточку идеи на витрине), `Редактировать` (открывает форму редактирования идеи; доступно для Черновик/На доработке).
...
- Табличный реестр запросов:
  - Колонки: `Запрос` (название + бейдж `+N`), `Статус` (круглый индикатор `На сборе идей`/`Закрыт`), `Направление` (цветной бейдж), `Дедлайн` (бейдж с датой `ДД.ММ.ГГГГ` или значением `Просрочено`), `Идей всего`, `Новых`, `Последнее обновление`.
  - Действия в строке: `Просмотр` (открывает карточку запроса на витрине), `Редактировать` (сразу открывает форму редактирования), `Закрыть` (при необходимости). Кнопки располагаются в отдельной колонке; `Просмотр` и `Редактировать` присутствуют всегда.
- В раскрытом запросе:
  - Блок `Комментарии` (стейкхолдер, координатор, модератор, авторы) заменяет отдельные примечания.
  - Блок `Идеи по запросу` содержит таблицу компактных карточек идей с кнопками решений (`Одобрить`, `Отправить на доработку`, `Отклонить`).
...
- Фильтры-чипсы: `На рассмотрении`, `На доработке`, `Отклонено`, `Одобрено`, `Архив` — применяются к обеим колонкам и позволяют отфильтровать карточки по статусам.
