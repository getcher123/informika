...
- Поля повторяют структуру HTML-макета:
  ...
  5. `Основные этапы реализации идеи*` — textarea, подсказка «Напишите основные этапы, которые необходимы для реализации вашего запроса».
  6. `Ожидаемый результат*` — textarea, подсказка «Напишите ожидаемый результат от реализации вашего запроса».
  7. `Есть ли у вас наработки по данному запросу?*` — радиокнопки `Да`/`Нет`.
  8. `Файлы к запросу` — drag&drop-зона (`file-upload`): кнопка выбора + подпись «Выберите файлы или перетащите их сюда», поддерживается множественная загрузка.
...
- **Карточка заявки (унифицированная)**:
  ...
  У участника доступна кнопка `Отозвать` если он создавал заявку через кнопку "Вступить в команду".
...
- Карточка уведомления (одна строка):
  ...
  - Метаданные справа: только время и (при необходимости) бэйдж дедлайна (`Срок завтра`, `Просрочено`). 
...
- Представление по стадиям: единая колонка/список `Одобрена`; дальнейшие этапы (включая подготовку к ПК/НТК) сопровождаются в Bitrix24 и не добавляют новых индикаторов на витрине.
- Карточка (минималистичная):
  - Верх: название, круглый индикатор `Одобрена`, индикация `+N` при наличии новых событий.
  - Средний блок: краткое описание (до 2 строк) и строка «Запрос: <название>».
  - Нижний блок: компактные текстовые подписи «Команда: заполнено X обязательных ролей» и «Просрочено задач: нет данных».
