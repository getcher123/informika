...
├── docs/
│   └── new-mockup-guide.md       # Пошаговый гайд по созданию новых страниц
...
13. **team-requests.html** - Командные запросы
    - Карточки для формирования проектных команд
    - Использование статусов и бейджей ролей
14. **stake-holder-dashboard.html** - Дашборд стейкхолдера
    - Сводные показатели запросов и последних действий
    - Таблица для быстрой навигации по инициативам
...
Страница `ui-kit.html` содержит не только перечень атомарных компонентов, но и целые сценарии построения страниц.

- **Вертикальные отступы** — гайд по utility-классам (`.section`, `.search`, `.filters`, `.back-link`), которые задают ритм между блоками.
- **Градиентный фон кабинета** — демо нового эффекта с размытыми окружностями + CSS-сниппет для класса `.body--gradient`.
- **Личный кабинет** — последовательность «табы → заголовок → поиск → фильтры → карточки» показана визуально вместе с сайдбаром.
- **Детальные страницы** — пример макета `detail-layout` с ссылкой «Назад», hero-карточкой, контентными секциями и правой колонкой.
- **Команды и оценки** — готовые блоки `.team-card`, `.team-grid`, `.evaluation-table` с тремя цветами строк.
- **Базовые компоненты** — header, sidebar, кнопки, формы, табы, карточки, бейджи, фильтры и т.д., каждый с перечнем классов и live-примером.

Все интерактивные блоки (табы, фильтры, карточки) подключены к `app.js`, поэтому можно сразу проверить поведение перед переносом на страницу. Полный свод правил и повторно используемых классов — в [`UNIFICATION_GUIDE.md`](./UNIFICATION_GUIDE.md).
...
### 2. Компоненты и блоки

Подробные сниппеты хедера, футера, сайдбара, форм, карточек, градиентного фона и порядок блоков страниц описаны в [`UNIFICATION_GUIDE.md`](./UNIFICATION_GUIDE.md). Рекомендации:

- переносите блоки из UI Kit без изменений (header/footer/sidebar/detail-layout и т.д.);
- формы и кнопки берите из `components.css`, чтобы не плодить локальные стили;
- `body--gradient` включайте на всех страницах кабинета — правила есть в гайде;
- новые экраны сверяйте с каркасами «Личный кабинет» и «Детальные страницы» в UI Kit.
...
## 🧭 Гайд по созданию новых макетов

- Подробный процесс описан в [`docs/new-mockup-guide.md`](./docs/new-mockup-guide.md): подготовка окружения, структура файлов, подключение компонентов, чек-лист требований и правила передачи артефактов.
- Гайд тесно ссылается на `docs/html-mockup-requirements.md`, `UNIFICATION_GUIDE.md` и `ui-kit.html`, поэтому держите их под рукой при разработке.
- Все новые повторно используемые блоки сначала документируются в UI Kit, после чего применяются на страницах.

Следование этому сценарию гарантирует, что новый макет сразу совместим с Bitrix-шаблонами и не потребует внеплановой доработки.
