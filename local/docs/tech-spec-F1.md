...
Теги-интересы/ролей в профиле (при наличии): #работник_Мособлгаза, #ментор, #стейкхолдер, #эксперт (отображаются, но в Ф1 функционал менторов/экспертов не активируется).
...
- Запрос на инновацию (инициирует стейкхолдер):
  - Поля: id, название, проблема/цель, критерии принятия, сроки сбора идей (дедлайн), владелец (стейкхолдер), направление, вложения (PDF/DOCX/XLSX ≤ 10 МБ), теги, дата публикации, статус.
  - Статусы: Черновик → На сборе идей → Закрыт (автоматически по дедлайну или вручную).
- Идея (привязана к запросу):
  - Поля (минимум): id, заголовок (≤ 100 символов), краткое описание (≤ 1000 знаков), запрос‑связка, автор, направление (наследуется/может уточняться), ожидаемый эффект (свободный текст), вложения (PDF/DOCX/XLSX ≤ 10 МБ), теги, даты, история модерации, статус.
  - Статусы и переходы:
...
- Пользователь/Профиль: ФИО, организация/подразделение/должность (при наличии), контакты (телефон, e‑mail), дата рождения (опц.), теги ролей, пароль (хранение по правилам платформы), фото (опц.).
...
- Общий хедер: МЕДИАЦЕНТР, ПРОЕКТНАЯ БИРЖА, КОНКУРС, ОБУЧЕНИЕ, ВОЙТИ/ПРОФИЛЬ (иконка аватара при авторизации).
...
- Каталог запросов:
  ...
  - Карточка запроса:
    - Тег-направление + статус-чип.
    - Название, краткое описание.
    - Дедлайн (если активен), владелец (ФИО/отдел).
    - Кнопка «Подробнее».

- Карточка запроса (страница):
  - Шапка: тег-направление, статус-чип, заголовок, владелец (ФИО/подразделение).
  - Разделы: Проблема/цель, Критерии принятия решения, Сроки (публикация, дедлайн), Вложения (список: имя, тип, размер), Теги.
...
- Карточка идеи (страница):
  - Шапка: тег-направление, статус-чип («Черновик»/«На модерации»/«На доработке»/«Принята»/«Отклонена»), заголовок, привязка к запросу (ссылка), автор (ФИО).
...
- Личный кабинет (общий для участника и стейкхолдера):
  ...
  - Вкладка «Мои заявки»:
    ...
    - Карточка элемента (Идея): тег-направление + статус-чип; название, краткое описание; «Ваша роль: Автор»; кнопка «Подробнее»; для статусов «Черновик/На доработке» — «Редактировать».
    - Карточка элемента (Запрос): тег-направление + статус-чип; название, краткое описание; «Ваша роль: Владелец запроса»; дедлайн (если активен); кнопки «Подробнее», «Изменить» (в Черновике/На сборе идей), «Закрыть сбор» (если активен).
  - Вкладка «Профиль»:
...
- Таблицы/коллекции:
  - Requests: id, title, problem, criteria, deadline, owner_id, direction, tags, files[], status, created_at, published_at, closed_at.
  ...
  - Users: id, fio, org, dept, position, phone, email, dob, roles[], tags[], avatar_url, created_at, last_login.
...
- Проекты, доски задач, роли трекер/ментор/эксперт, научные советы/ПК — вне Ф1.
- Сложная аналитика, публичные отчёты и дашборды — вне Ф1.
- Архивация проектов и событий/календарь — вне Ф1.
...
- Роли:
  ...
  - Модератор может модераровать запросы и идеи, возвращать на доработку, отклонять и публиковать; решения и комментарии фиксируются.
