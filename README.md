...
Сделай разработчика владельцем `local`, чтобы править файлы без root:
...
- Включили SSH и создали пользователя `dev` — дальше VS Code будет работать через него.
...
### Что делаем по итогу
- VS Code подключается к ВМ по Remote-SSH под пользователем `dev`.
...
- Пользователь `dev` создан, SSH доступ есть:
...
Готово ✅
Дальше (Шаг 5) настроим репозиторий/ветвление/шаблоны Issues/PR, CODEOWNERS и защиту веток, чтобы процесс разработки был предсказуемым и прозрачным.

## Шаг 5 из 7. Репозиторий, ветвление, шаблоны Issues/PR, CODEOWNERS и защита веток
...
- Шаблоны GitHub Issues (задачи/баги) и PR (как описывать изменения).
- CODEOWNERS → автозапрос ревью у нужных людей.
- Branch protection для main → нельзя случайно сломать прод.
- Быстрый «канбан» на GitHub Projects (по желанию).

### 1) Базовая структура репозитория
В корне (то, что уже частично сделали/сделаем):

```
/local/                      # код Bitrix (единственное место правок)
/.github/
  workflows/ci.yml           # из шага 4
  ISSUE_TEMPLATE/
    bug.md
    feature.md
...
### 3) Шаблоны Issues (задач) и PR
Создай каталог и файлы:
...
## Скриншоты/видео (если уместно)

## Проверка
- [ ] npm run fmt:write
- [ ] npm run lint:js && npm run lint:css
- [ ] composer cs:write && composer stan
- [ ] Самопроверка на staging/локально

## Связанные задачи/тикеты
Closes #<номер issue>
```

Зачем шаблоны: не надо «думать, что писать». Все важные поля всегда есть — ревью идёт быстрее.
...
### 6) Лейблы, milestone и Projects (быстрый канбан)
#### 6.1 Лейблы (Labels)
Создай пару базовых:
...
Это помогает фильтровать задачи.

#### 6.2 Milestones (вехи)
Например: `MVP (Фаза 1)` — дедлайн и список тикетов, чтобы видеть прогресс.

#### 6.3 GitHub Projects (опционально)
Создай project board `Innovation Dev` с колонками:

```
Backlog, Ready, In Progress, PR, Review, Staging, Done
```

И привяжи Issues — получится простой канбан.

### 7) CONTRIBUTING.md (короткий гайд для всех)
...
### 8) Мини-ритуалы и «готов к работе»
...
### 9) Чек-лист готовности шага
- Есть README.md с инструкцией быстрого старта.
- Добавлены шаблоны Issues/PR для багов и фич.
...
- Настроены Labels, Milestones и (опционально) Project board для визуального контроля задач.

### Простыми словами — зачем это всё
- Шаблоны экономят время: не надо думать, что писать, все важные поля уже есть.
...
- Labels, Milestones, Projects помогают видеть, что в работе, что готово и что приоритетно, без лишней болтовни.
